...
아이디어 → 콘솔 업로드 직전까지 7단계를 순차 실행한다. 각 단계의 본문은 호출되는 `/ait-*` 스킬이 자기 책임으로 읽는다 — 이 스킬은 **오케스트레이션만** 담당.
...
| 2 | 리소스 점검 | `/ait-assets` | appName 확정, 브랜드 컬러 |
...
| 6 | 검수 | `/ait-review` full | 자동 수정. **커밋** |
| 7 | 빌드 & 배포 | `/ait-build` | typecheck→lint→build, 콘솔 업로드 안내 |

## 운영 원칙

- **핸드오프**: 앞 단계에서 확정된 정보(appName, 앱 유형, 브랜드 컬러, 설치 스킬 목록)는 뒷단계에서 재질문하지 않는다.
- **에러 복구**: typecheck/lint 에러는 자동 수정 재시도. 빌드 실패 시 STEP 6 부터 재점검. 대화 중단 시 `apps/<app-name>/` 존재 여부로 시작 단계 판단.
