...
- **jq 또는 python3 없음** — gpt-image-1 캐릭터·일러스트 생성 시에만 필요 (b64 디코딩). 둘 중 하나만 있으면 됨. macOS: `brew install jq`

### 환경변수 (🟡 Warn)

- **.env 없음** — `cp .env.example .env` 후 필요한 키 입력
- **OPENAI_API_KEY 미설정** — gpt-image-1 캐릭터·일러스트 생성 기능에만 영향. 일반 로고·썸네일·스크린샷 생성엔 무관
