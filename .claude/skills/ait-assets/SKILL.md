...
- `[Dashboard session contract]` 가 있으면: 다음 스킬(`/ait-scaffold`) 호출 권유 문구 금지. 대시보드가 알아서 다음 단계를 보여준다.
- CLI 세션일 때만 "다음은 /ait-scaffold 로 스캐폴딩 가능" 같은 1줄 안내 허용.
