# Barreleye Documentation

> 앱인토스 미니앱 모노레포 문서

## Quick Links

- [AGENTS.md](../AGENTS.md) - AI 에이전트 컨텍스트 맵
- [Roadmap](roadmap.md) - 목표 · 설계 원칙 · 단계별 우선순위

## Architecture

- [Overview](architecture/overview.md) - 모노레포 구조
- [Dependency Layers](architecture/dependency-layers.md) - 레이어 규칙

## Guides

- [Create App](guides/create-app.md) - 새 미니앱 생성
- [Dependencies](guides/dependencies.md) - 의존성 관리

## Conventions

- [Code Style](conventions/code-style.md) - 코드 스타일 가이드
- [Commits](conventions/commits.md) - 커밋 컨벤션

## Troubleshooting

- [Common Errors](troubleshooting/common-errors.md) - 자주 발생하는 에러 해결

## External Resources

- [앱인토스 개발자 센터](https://developers-apps-in-toss.toss.im)
- [TDS React Native](https://tossmini-docs.toss.im/tds-react-native)
- [TDS Mobile (Web)](https://tossmini-docs.toss.im/tds-mobile)
